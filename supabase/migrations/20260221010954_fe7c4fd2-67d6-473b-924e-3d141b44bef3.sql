...
-- Processes an entire sale atomically: sale, items, stock updates, financial splits, 
-- payment reminder, shipping expense. One round-trip, full rollback on failure.
...
BEGIN
  ...
  IF payload->'stock_updates' IS NOT NULL AND jsonb_array_length(payload->'stock_updates') > 0 THEN
    ...
    LOOP
      IF _stock_update->>'variant_id' IS NOT NULL AND _stock_update->>'variant_id' != '' THEN
        -- Update variant stock (trigger will sync product stock)
  ...
  -- 6. Create payment reminder (deferred payments)
  IF payload->'payment_reminder' IS NOT NULL AND payload->>'payment_reminder' != 'null' THEN
  ...
  -- Return the sale ID for client use
